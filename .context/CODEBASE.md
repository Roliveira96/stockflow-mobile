...
**Dashboard concluído.** Tela inicial pós-login com acolhimento animado e métricas do dia. Ver seção "Dashboard" no fim.
...
### PIX só depois da conferência + envio por WhatsApp (pedido do usuário)
- **`PagamentoPix` agora é um fluxo em etapas.**
  1. **Bloqueado:** mostra "Confira os itens: N de M" com o botão "Gerar pagamento PIX" desabilitado, até **todos** os itens estarem marcados como conferidos/embalados.
  2. **Gerando:** ao tocar, aparece "Gerando cobrança PIX…" com o ícone pulsando (`Animated.loop`) por 1,2 s.
  3. **Gerado:** o QR, o valor, a chave e o "copia e cola" entram com `Animated.spring` (opacidade + escala 0,85 → 1).
- A tela `pedido-caixa` controla se o PIX foi gerado por uma **chave de cobrança** `id:total` (`pixGeradoPara`). Se o total muda depois (edição, item de balcão), a chave não bate mais e o QR some. Assim nunca aparece um QR com valor desatualizado. Em PIX, **"Receber" fica desabilitado até o QR ser gerado**. `aoGerar` é `useCallback`, senão cada nova renderização reiniciaria o temporizador da animação dentro do efeito.
- **"Enviar no WhatsApp"** (abaixo do QR gerado): `utils/whatsapp.ts` monta a mensagem (saudação com o primeiro nome, número do pedido, itens com totais, balcão, subtotal, desconto, total em negrito, PIX copia e cola, chave e aviso de demonstração) e o link `https://wa.me/55<telefone>?text=...`; sem telefone, fica `https://wa.me/?text=...`, para escolher o contato.
  - **Armadilha encontrada:** o `Linking.openURL` do **`expo-linking` na web faz `window.location = url`**, ou seja, troca a página do app pelo WhatsApp (conferido lendo `node_modules/expo-linking/build/RNLinking.web.js`). Por isso aqui se usa o `Linking` do `react-native`, que no `react-native-web` abre com `window.open(url, "_blank")`; no nativo os dois se comportam igual.
- **Validado com Playwright (Pixel 7):** 0/2 e 1/2 → "Gerar" desabilitado; 2/2 → liberado; a animação aparece; o QR aparece e o "Receber" libera; o WhatsApp abre em nova aba (redireciona para `api.whatsapp.com/send/` com o telefone) e a mensagem decodificada está completa; ao adicionar um item de balcão o QR some e o "Receber" volta a ficar desabilitado. Sem erros.

## Dashboard

- **`src/app/(app)/dashboard.tsx`:** depois do login, `login.tsx` faz `router.replace` para `/dashboard?intro=1`, que mostra a saudação ("Bom dia, <nome>!") e "Estamos preparando o seu ambiente..." com barra de progresso animada, e depois os indicadores. O menu lateral ganhou o item "Dashboard" (`telaAtiva: "dashboard"`).
- **Dados:** carrega em paralelo `listarPedidos`, `buscarProdutosPaginado`, `buscarLotesComValidadeProxima` e `listarCategorias`, cada um com `catch` próprio para uma falha não derrubar a tela. Os cálculos (vendas de hoje, ticket médio, itens vendidos, pedidos aguardando, vendas do operador, divisão por forma de pagamento, pedidos recentes) ficam em `src/utils/dashboard.ts`.
- **Estilos:** `src/styles/dashboard.styles.ts`. As cores de fundo dos ícones são variantes do `StyleSheet` (`iconeFundoPrimaria`/`Sucesso`/`Neutro`/`Alerta`/`Perigo`), sem estilo inline; só os valores de `Animated` ficam no array de `style`, como no `Toast` e no `MenuLateral`.

## Ajustes para a defesa (conferência com o edital)

- **Exclusão no navegador:** `Alert.alert` com botões não faz nada no `react-native-web`, então o "Remover" não funcionava no navegador. `src/utils/confirmacao.ts` (`confirmarAcao`) usa `window.confirm` na web e `Alert.alert` no nativo; aplicado na listagem e em `visualizar-produto.tsx`.
- **Campo vazio mostra erro (req. 5, "vazio ou formato inadequado"):** no `ProdutoForm`, nome, código de barras, quantidade e preço de custo marcam o campo como tocado ao digitar ou sair dele (`onBlur`). Se o campo tocado ficar vazio, aparece "… é obrigatório" (o preço de custo ganhou erro próprio, igual ao preço de venda). Campos nunca tocados continuam sem erro, para o formulário não abrir todo vermelho.
- **Carregamento na montagem (req. 6 e 9):** enquanto o `AuthContext` lê o AsyncStorage, o `_layout.tsx` raiz mostra um `ActivityIndicator` (estilo `carregandoSessao` em `src/styles/layout.styles.ts`) em vez de `return null`.
